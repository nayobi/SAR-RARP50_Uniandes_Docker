...
from . import head_helper, resnet_helper, stem_helper, swin_transformer
...
class SlowFast(nn.Module):
    ...
    def _construct_network(self, cfg):
        ...
        for idx, task in enumerate(self.tasks):
            if task in ['tools','actions','segmentation']:
                ...
                extra_head = head_helper.ResNetBasicHead(
                    ...
                    enable_extra_heads=self.enable_extra_heads,
                )
    ...
    def forward(self, x, bboxes=None, features=None):
        ...
        for task in self.tasks:
            ...
            if task in ['tools','actions','segmentation']: #== 'tools' or task == 'actions':
                out_features, multi_faster = extra_head(x, bboxes, features)
                out[task].append(out_features)
            else:
                out[task].append(extra_head(x))
        
        return out, multi_faster
...
class MViT(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # Natalia Prepare PSI-AVA tasks
        self.tasks = cfg.TASKS.TASKS
        self.enable_detection = cfg.DETECTION.ENABLE
        self.num_classes = cfg.TASKS.NUM_CLASSES
        self.act_fun = cfg.TASKS.HEAD_ACT
        self.test_enable = cfg.TEST.ENABLE
        ...
        # Natalia Following MocoV3, initializing with random patches stabilize optimization
        ...
        self.embed_dim = dim_out #Natalia embed_dim = dim_out
        cfg.PRETRAIN.EXPANSION = self.embed_dim #Natalia
        self.norm = norm_layer(self.embed_dim)
        pool_size = _POOL1[cfg.MODEL.ARCH] #Natalia from here -----------------------------
    ...
    def forward(self, x, bboxes=None, features=None, keep_box=None):
        # breakpoint()
        out = {k:[] for k in self.tasks} #Natalia
        x = x[0]
        ...
        for task in self.tasks:
            ...
            if task in ['tools','actions','segmentation']: #== 'tools' or task == 'actions':
                ...
                #     out_features, multi_faster = extra_head(t, thw, bboxes[keep_box], features[keep_box])
                # else:
                out_features, multi_faster = extra_head(t, thw, bboxes, features)
                ...
                if self.cls_embed_on:
                    ...
                    t = x.mean(1)
                if self.test_enable:
                    out[task].append((extra_head(t),t))
                else:
...
class Projector(nn.Module):
    def __init__(self, cfg):
        ...
        input_size=cfg.PRETRAIN.EXPANSION
...
class Postprocessor(nn.Module):
    def __init__(self, num_layers, model,
                    num_heads=8, dim_ff=768, drop=0.1, act_fun='relu',
                    lne=1e-05, batch_first=False, bias=True, bidirect=True, contin=0) -> None:
        super(Postprocessor, self).__init__()

        self.model = model
        self.contin = contin>0
        if model=='gru':
            self.gru = nn.GRU(768, dim_ff, num_layers, bias, batch_first, drop, bidirect)
        if model=='lstm':
            self.lstm = nn.LSTM(768, dim_ff, num_layers, bias, batch_first, drop, bidirect)
        if model=='trans':
            encoder_layer = nn.TransformerEncoderLayer(768, num_heads, dim_ff, drop, act_fun, lne, batch_first)
            self.encoder = nn.TransformerEncoder(encoder_layer, num_layers)
            self.pos_encoder = PositionalEncoding(768)

        mlp_dim = dim_ff if model=='trans' or not bidirect else dim_ff*2
        self.mlp = nn.Sequential(nn.Linear(mlp_dim,8,bias=True),)
        if self.contin:
            self.seq_mlp = nn.Sequential(nn.Linear(mlp_dim,contin,bias=True),)
            self.softmax = nn.Softmax(dim=2)
    
    def forward(self, seqs, masks):
        # breakpoint()
        seqs = torch.permute(seqs,(1,0,2))
        if self.model == 'trans':
            seqs = self.pos_encoder(seqs)
            x = self.encoder(seqs,src_key_padding_mask=masks)
