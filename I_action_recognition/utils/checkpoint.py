...
def make_checkpoint_dir(path_to_job):
    ...
    """
    checkpoint_dir = os.path.join(path_to_job, "stuff_AR", "checkpoints")
...
def save_checkpoint(path_to_job, model, optimizer, epoch, cfg, scaler=None, scheduler=None, projector=None):
    """
    Save a checkpoint.
    Args:
    ...
    if projector is not None:
        checkpoint["projector_state"] = projector.state_dict()
...
def load_checkpoint(
    ...
    if projector:
        ps = projector.module if data_parallel else projector
    if convert_from_caffe2:
        ...
        if projector is not None:
            projector_state_dict_3d = (
                projector.module.state_dict() if data_parallel else projector.state_dict()
            )
            checkpoint["projector_state"] = normal_to_sub_bn(
                checkpoint["projector_state"], projector_state_dict_3d
            )
        if inflation:
            ...
            if projector:
                inflated_projector_dict = inflate_weight(
                    checkpoint["projector_state"], projector_state_dict_3d
                    )
                ps.load_state_dict(inflated_projector_dict, strict=False)
        else:
            if clear_name_pattern:
                for item in clear_name_pattern:
                    ...
                    if projector:
                        projector_state_dict_new = OrderedDict()
                        for k in checkpoint["projector_state"]:
                            if item in k:
                                k_re = k.replace(item, "")
                                projector_state_dict_new[k_re] = checkpoint[
                                    "projector_state"
                                ][k]
                                logger.info("renaming: {} -> {}".format(k, k_re))
                            else:
                                projector_state_dict_new[k] = checkpoint["projector_state"][
                                    k
                                ]
                        checkpoint["projector_state"] = projector_state_dict_new
            ...
            if projector:
                pre_train_dict = checkpoint["projector_state"]
                model_dict = ps.state_dict()
                # Match pre-trained weights that have same shape as current model.
                pre_train_dict_match = {
                    k: v
                    for k, v in pre_train_dict.items()
                ...
                if not_load_layers:
                    for k in not_load_layers:
                        logger.info("Network weights {} not loaded.".format(k))
                # Load pre-trained weights.
                ps.load_state_dict(pre_train_dict_match, strict=False)
            epoch = -1
...
def load_pretrain_checkpoint(cfg, model, projector, optimizer, scheduler):
    """
    Loading checkpoint logic for training.
    """
    if cfg.TRAIN.AUTO_RESUME and has_checkpoint(cfg.OUTPUT_DIR):
        ...
        logger.info("Load from last checkpoint, {}.".format(last_checkpoint))
        checkpoint_epoch = load_checkpoint(
            last_checkpoint, model, cfg.NUM_GPUS > 1, optimizer, scheduler=scheduler, projector=projector
        )
        start_epoch = checkpoint_epoch + 1
    elif cfg.TRAIN.CHECKPOINT_FILE_PATH != "":
        logger.info("Load from given checkpoint file.")
